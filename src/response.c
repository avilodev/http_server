 ...
 *
 * @see send_error_response(), get_content_type()
 */
int send_file_response(Client* client, struct Node* cache_node) {
    ...
    // Headers
    char* content_type = get_content_type(client->full_path);
 ...
 *
 * Analyzes file path extension and returns the appropriate MIME type string.
 * Supports common web content types (HTML, CSS, JS, images, video) and
 * defaults to "application/octet-stream" for unknown types.
 *
 * @param path File path or filename containing extension
 *
 * @return Dynamically allocated string containing MIME type
 *
 * @note Returns "application/octet-stream" if path is NULL or has no extension
 * @note Performs case-insensitive extension matching
 * @warning Caller must free the returned string
 *
 * @see send_file_response()
 */
char* get_content_type(const char* path) {
    if (!path) return strdup("application/octet-stream");
